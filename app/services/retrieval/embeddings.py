...
BATCH_SIZE = 50
_GEMINI_DIM = 3072
_FALLBACK_DIM = 768 #all-mpvet-base-v2
...
def _probe_gemini():
    """Try one embed call to verify is reachable. Returns model or None."""
...
def _load_fallback():
        ...
        logfire.info("Loading sentence-transformers fallback ")
        ...
        for attempt in range(4):
            try:
                ...
                if is_rate_limit and attempt<3:
                    ...
                    logfire.warning(f"Gemini embedding failed: {e}")
                    raise 
...
def embed_query(query: str) -> list[float]:
    _init()
    if _model_type == "gemini":
        return _active_model.embed_query(query) #this will be in langchain's googlegenerativeaiembeddings class not our embed_query function what we wrote in
