...
def parse_pdf(file_path: str):
    """"
    Extracts text from a PDF locally using pypdf.
    Falls back to pdfplumber for pages that yield no text (eg. image-heavy pages).
    ...
    with logfire.span("PDF Parsing",filename = file_path):
        try:
            ...
            logfire.error(f"Error parsing this PDF file {file_path} : {e}")
            raise e
